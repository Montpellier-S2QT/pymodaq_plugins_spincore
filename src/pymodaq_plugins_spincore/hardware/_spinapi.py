...
# Defines for timing intervals
ns = 1.0
us = 1000.0
ms = 1000000.0

# Defines for frequency values
MHz = 1.0
kHz = 0.001
Hz = 0.000001
...
# Define _SpinAPI class for namespacing
class _SpinAPI:
    ...
    def pb_overflow(self, *args):
        return spinapi.pb_overflow(*args)


    def pb_scan_count(self, *args):
        return spinapi.pb_scan_count(*args)


    def pb_set_num_points(self, *args):
        return spinapi.pb_set_num_points(*args)


    def pb_set_radio_control(self, *args):
    ...
    def pb_write_register(self, address, value):
    ...
    def pb_inst_dds(self, FREQ, TX_PHASE, TX_ENABLE, PHASE_RESET, FLAGS, INST, INST_DATA, LENGTH):
        return self.pb_inst_radio(
            FREQ, 0, 0, TX_PHASE, TX_ENABLE, PHASE_RESET, 0, FLAGS, INST, INST_DATA, LENGTH
        )


    def pb_inst_radio_shape(self, *args):
        t = list(args)
    ...
    def pb_inst_dds_shape(
        ...
        return self.pb_inst_radio_shape(
            FREQ,
            0,
            0,
            TX_PHASE,
    ...
    def pb_sleep_ms(self, mlsc):
        return spinapi.pb_sleep_ms(mlsc)


    def pb_get_data(self, num_points, real_data, imag_data):
    ...
    def pb_get_data_direct(self, num_points, data):
    ...
    def pb_unset_radio_control(self, ctrl):
        return spinapi.pb_unset_radio_control(ctrl)


    def pb_write_felix(self, *args):
        t = list(args)
    ...
    def pb_setup_filters(self, *args):
        t = list(args)
        # Argument 0 must be a double
        t[0] = ctypes.c_double(t[0])
        args = tuple(t)
    ...
    def pb_inst_radio_shape_cyclops(self, *args):
        t = list(args)
    ...
    def pb_fft_find_resonance(self, num_points, SF, SW, real_data, imag_data):
    ...
    def pb_write_ascii(self, fname, num_points, SW, real_data, imag_data):
    ...
    def pb_write_ascii_verbose(self, fname, num_points, SW, SF, real_data, imag_data):
        # Convert the file name to a C-style string
        c_fname = ctypes.c_char_p(fname.encode())
    ...
    def pb_write_jcamp(self, fname, num_points, SW, SF, real_data, imag_data):
        # Convert the file name to a C-style string
        c_fname = ctypes.c_char_p(fname.encode())
    ...
    def pb_set_scan_segments(self, num_segments):
        # Call the C function
