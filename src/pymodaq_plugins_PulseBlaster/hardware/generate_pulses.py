...
def generate_repeating_pulses(
    ...
    if min_instruction_len <= 0:
        raise ValueError(
            f"min_instruction_len must be positive, got {min_instruction_len}"
        )
    if nr_channels <= 0:
    ...
    # -------------------------------
    # Detect and separate constant-high signals
    # -------------------------------
    constant_channels = []
    variable_signals = []
    for s in signals:
        if getattr(s, "duty_cycle", None) is not None and s.duty_cycle >= 1.0:
            logging.info(
                f"Signal '{getattr(s, 'name', '?')}' has duty_cycle=1.0 → will be set constant high."
            )
            constant_channels.extend(s.channels)
        else:
            variable_signals.append(s)
    signals = variable_signals

    if not signals and not masking_signals:
        # Only constant-high signals, no pulses
        logging.info(
            "Only constant-high signals detected; generating static instruction."
        )
        chs = [1 if i in constant_channels else 0 for i in range(nr_channels)]
        return InstructionSequence(
            [
                Instruction(
                    label="", flags=chs, duration=100, opcode=Opcode.BRANCH, inst_data=0
                )
            ]
        )

    # -------------------------------
    # Core pulse sequence generation
    # -------------------------------
